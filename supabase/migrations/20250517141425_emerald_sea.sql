/*
  ...
  1. New Tables
    ...
    - `categories`: Manages document categories
    - `document_categories`: Junction table for document-category relationships

  2. Security
    - Enable RLS on all tables
    - Add policies for document management
    - Add policies for category access
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'All users can read categories'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can manage document categories for their documents'
  ) THEN
